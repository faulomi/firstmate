...
SESSION=$("$ROOT/bin/fm-herdr-lab.sh" name ledger4)
...
ctl() { FM_HOME="$H" FM_ROOT_OVERRIDE="$ROOT" FM_STATE_OVERRIDE="$H/state" FM_DATA_OVERRIDE="$H/data" FM_CONFIG_OVERRIDE="$H/config" "$ROOT/bin/fm-control.sh" "$@"; }
echo; echo "=== S4d ship local-only, real claude (haiku) in lab, fm-control relaunch twice -> relaunches=2, attempt 3"
brief lshipf 'Delivery contract: mode=local-only'
spawn lshipf "$P" --harness claude --mode local-only --yolo off --model haiku --effort low
for n in 1 2; do
  sleep 25
  ctl lshipf relaunch --note "lab retry $n" >"$TMP_ROOT/rl$n.out" 2>&1; echo "fm-control relaunch $n rc=$?"; tail -4 "$TMP_ROOT/rl$n.out"
  echo "--- state/lshipf.meta after relaunch $n"; grep -E '^(harness|model|effort|spawned_at|relaunches|spawn_gen)=' "$H/state/lshipf.meta"
done
st lshipf "done [at=$(date +%s)]: ok"
teardown lshipf; ledger
