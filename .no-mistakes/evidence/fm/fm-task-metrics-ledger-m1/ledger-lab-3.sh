...
SESSION=$("$ROOT/bin/fm-herdr-lab.sh" name ledger3)
...
FB="$TMP_ROOT/fakebin"; mkdir -p "$FB"; printf '#!/bin/sh\necho fake-claude "$@" | head -c 200; echo; exit 0\n' > "$FB/claude"; chmod +x "$FB/claude"
export PATH="$FB:$PATH"
echo; echo "=== S4c ship local-only, stub claude exits, fm-spawn --relaunch twice -> relaunches=2, attempt 3"
brief lshipe 'Delivery contract: mode=local-only'
spawn lshipe "$P" --harness claude --mode local-only --yolo off --model opus --effort low
PANE=$(grep '^herdr_pane_id=' "$H/state/lshipe.meta" | cut -d= -f2-)
for n in 1 2; do
  sleep 4
  echo "pane capture:"; fm_herdr_lab_raw "$SESSION" pane read "$PANE" --lines 5 2>/dev/null | tail -5
  FM_SPAWN_NO_GUARD=1 FM_HOME="$H" FM_ROOT_OVERRIDE="$ROOT" "$ROOT/bin/fm-spawn.sh" lshipe --relaunch >"$TMP_ROOT/rl$n.out" 2>&1; echo "relaunch $n rc=$?"; tail -4 "$TMP_ROOT/rl$n.out"
  echo "--- state/lshipe.meta after relaunch $n"; grep -E '^(harness|model|effort|spawned_at|relaunches|spawn_gen)=' "$H/state/lshipe.meta"
done
st lshipe "done [at=$(date +%s)]: ok"
teardown lshipe; ledger
