...
root=pathlib.Path.cwd(); d=root/'.test-memory-phase/live-profile'; d.mkdir(parents=True,exist_ok=True)
...
try:
 ...
 for kind in ['ship']:
  ...
  args=[str(root/'bin/fm-spawn.sh'),task,str(proj),'--harness','codex','--model','gpt-5','--effort','high']
  args+=['--scout'] if kind=='scout' else ['--mode','local-only','--yolo','off']
  ...
  rows=[x for x in procs.stdout.splitlines() if ('codex ' in x and str(d) in x)]
