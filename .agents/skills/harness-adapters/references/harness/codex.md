...
| Memory | Crewmate and scout launches, including raw Codex commands with quoted or path-qualified executables and supported wrapper prefixes, receive `--disable memories`; `fm-spawn.sh` owns the accepted raw syntax and refuses unresolved commands or policy overrides before launch, while secondmate launches and personal configuration keep their existing defaults; verified on 2026-09-09 with codex-cli 0.153.4: `codex features list --disable memories` and `codex -c 'features.memories=false' features list` both report `memories stable false`. |
